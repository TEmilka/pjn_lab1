...
SLEEP_MAX = 30.00
MAX_PAGES = 999
TARGET_MIN_RECORDS = 3  # zmień na 100 do finalnego uruchomienia
...
def next_page_url(current_url: str, page_index: int) -> str:
    from urllib.parse import urlencode, urlparse as _urlparse, urlunparse, parse_qsl
    pr = _urlparse(current_url)
...
def parse_product_page(html: str, url: str) -> ProductRow:
    ...
    # --- heurystyki z tytułu (uzupełnienia) ---
    ...
    if not color and name:
        # fallback: spróbuj z tytułu po wzorcach PL/EN
        color = guess_color_from_text(name)
